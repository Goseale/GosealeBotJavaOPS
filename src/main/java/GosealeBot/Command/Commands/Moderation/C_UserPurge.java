...
public class C_UserPurge implements ICommand {
    ...
    public void handle(CommandContext ctx) {
        ...
        Member target;
        if (args.isEmpty()) {
            target = ctx.getMember();
        } else {
            if (!mentions.isEmpty()) {
                target = mentions.get(0);
            } else {
                try {
                    target = ctx.getGuild().getMemberById(args.get(0));
                } catch (Exception e) {
        ...
        channel.getIterableHistory()
                ...
                .thenApplyAsync((messages) -> {
                    List<Message> goodMessages = messages.stream()
                            ...
                            .filter(((message) -> {
                                return message.getAuthor().getId().equals(target.getId());
                            }))
    ...
    public String getHelp() {
        return "Bulk delete messages from a user in a text channel\n" +
                "Usage: " + Configuration.get("prefix") +getName()+ " @mention [amount]";
    }
